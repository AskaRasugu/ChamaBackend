...
const blogLoader = new DataLoader((eventIds) => {
  // console.log(userIds)
  return blogs(eventIds);
});
const commentLoader = new DataLoader((eventIds) => {
  // console.log(userIds)
  return comments(eventIds);
});
...
const blogs = async (eventIds) => {
  try {
    const events = await Blog.find({ _id: { $in: eventIds } });
    events.sort((a, b) => {
      return (
        eventIds.indexOf(a._id.toString()) - eventIds.indexOf(b._id.toString())
      );
    ...
    return events.map((event) => {
      return transformBlog(event);
    });
  } catch (err) {
    throw err;
  }
};
const comments = async (eventIds) => {
  try {
...
const singleBlog = async (eventId) => {
  try {
    const event = await blogLoader.load(eventId.toString());
    return event;
  } catch (err) {
    throw err;
  }
...
const transformBlog = (event) => {
  // console.log( event.user);
  return {
    ...event._doc,
    comments: comments.bind(this, event.comments),
    creator: useracc.bind(this, event.creator),
    createdAt: datetoString(event._doc.createdAt),
    updatedAt: datetoString(event._doc.updatedAt),
  };
};
const transformComment = (event) => {
  // console.log( event.user);
  return {
    ...event._doc,
    blog: singleBlog.bind(this, event.blog),
    user: useracc.bind(this, event.user),
    likes: creatorUser.bind(this, event.likes),
    createdAt: datetoString(event._doc.createdAt),
    updatedAt: datetoString(event._doc.updatedAt),
  };
};
