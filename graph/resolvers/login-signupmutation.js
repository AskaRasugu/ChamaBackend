...
const uri = `https://essaysinn.com`;
const mailgun = require("mailgun-js");
const domain = "mg.essaysinn.com";
...
module.exports = {
  ...
  login: async (parent, args, { res, req }) => {
    ...
    try {
      ...
      res.cookie("refreshtoken", refreshtoken, {
        ...
        httpOnly: true,
        secure: true,
        domain: "essaysinn.com", //set your domain
      });
  ...
  resetpassword: async (parent, args, context) => {
    ...
    const msg = {
      from: "support@essaysinn.com", // sender address
      to: `${user.email}`, // list of receivers
      subject: `Password Has Been Changed Via Essaysinn`, // Subject line
      text: `Essaysinn`, // plain text body
      html: `
  ...
  requestResetPassword: async (parent, args, context) => {
    ...
    const msg = {
      ...
      subject: `Password Change Request Via Essaysinn`, // Subject line
      text: `${link}`, // plain text body
      html: `

          <b> Hi ${existingEmail.username} </b> ,
          <p>
            You recently requested to reset the password for Essaysinn
  ...
  logoutuser: async (parent, args, context) => {
    ...
    try {
      ...
      const { ipAddress, deviceName, deviceBrand } = args;
      const userIp = ipAddress ? ipAddress : context.req.ip;

      // const index = user.deviceDetails.findIndex(
      //   (e) =>
      //     e.deviceName === deviceName && e.deviceBrand === deviceBrand && e.ipAddress === userIp
      // );
      // user.deviceDetails.splice(index, 1);
      // await user.save();
      ...
      context.res.cookie("refreshtoken", refreshtoken, {
        sameSite: "none",
        path: "/",
        expires: new Date(new Date().getTime() + 360),
        httpOnly: true,
        secure: true,
        // domain: 'themarketpalace.com', //set your domain
      });
      return user;
    } catch (err) {
      throw err;
